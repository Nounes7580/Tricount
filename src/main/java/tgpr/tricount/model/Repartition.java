...
public class Repartition extends Model {
    ...
    public List<Operation> getrepartitionsByOperation(Operation operation){
        return (List<Operation>) queryOne(Repartition.class, "select * from repartitions where operation=:operation",
                new Params("operation", operation));
    }
    ...
    public static int totalRepartition(int operationId){
       List<Repartition> repartitions = Operation.getByKey(operationId).getRepartitions();
       int sum = 0;
       for(Repartition repartition: repartitions){
           sum += repartition.weight;
       }
       return sum;
    }
    public Repartition(int userId, int weight) {
        this.userId=userId;
        this.weight=weight;
    }
